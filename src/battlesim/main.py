...
def main() -> None:
    ...
    small_font: pygame.font.Font = pygame.font.SysFont(c.UI.Font.FONT_NAME, 14)
    ...
    # Outer loop — keeps the game running for play again
    while True:
        if not ui.main_menu(screen, font):
            break

        player_one_team, player_two_team = draft_teams(screen, font, small_font)
        ...
        p1_shake_timer: int = 0
        p2_shake_timer: int = 0

        # Inner loop — one full game
        while running:
            _ = clock.tick(60)
            ...
            ui.draw_hp_bar(fielded_monsters[1], 50, 50, screen, font, small_font)
            ui.draw_hp_bar(fielded_monsters[0], 480, 320, screen, font, small_font)
            ...
            for event in pygame.event.get():
                ...
                if not game_over and event.type == pygame.KEYDOWN:
                    ...
                    if event.key in [pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4]:
                        ...
                        chosen = ui.switch_monster_menu(screen, font, small_font, active_team, active_player_monster, (current_turn % 2) + 1)
            ...
            if not game_over:
                for i in range(2):
                    if fielded_monsters[i].fainted:
                        ...
                        alive_monsters = [mon for mon in team if not mon.fainted]
                        if alive_monsters:
                            ...
                            if play_again:
                                running = False  # breaks inner loop → outer loop restarts
                            else:
                                pygame.quit()
                                sys.exit()
